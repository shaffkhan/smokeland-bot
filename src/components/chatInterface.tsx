...
export default function ChatInterface() {
  const [messages, setMessages] = useState<Message[]>([
    {
      type: "bot",
      content: "Hi, I'm Smokeland Bot! How can I assist you today?",
    },
  ]);
  ...
  const handleSendMessage = async (text: string, image: File | null) => {
    ...
    if (image) {
      ...
      setMessages((prev) => [
        ...prev,
        { type: "user", content: "Image uploaded", image: imageUrl },
      ]);
    } else {
    ...
    try {
      const response = await fetch(
        "https://smokeland-304959215088.us-central1.run.app/api/query",
        {
          method: "POST",
          headers: {
